...
public class WorkflowAllSteps {
	...
	public void a_request_is_prepared_to_create_an_employee() {
		...
		request = given().header("Content-Type", "application/json").header("Authorization", TokenGenerationSteps.token)
				.body(PayloadConstant.creatEmployeePayload());
	...
	@Then("the status cod for creating an employee is {int}")
	public void the_status_cod_for_creating_an_employee_is(int statusCode) {
	...
	@Given("request is prepared to get created employee")
	public void request_is_prepared_to_get_created_employee() {
		request = given().header("Content-Type", "application/json").header("Authorization", TokenGenerationSteps.token)
				.queryParam("employee_id", employeeID).log().all();
		System.out.println("------------------------NewClassSteps-------------------------------");

	}

	@When("a Get call is make to retieve the created employee")
	public void a_Get_call_is_make_to_retieve_the_created_employee() {

		response = request.when().when().get(APIConstants.GET_ONE_EMPLOYEE_ENDPOINT);
